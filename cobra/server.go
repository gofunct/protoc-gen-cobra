...
const generatedServerCodeVersion = 4

func init() {
	generator.RegisterPlugin(new(serverGen))
}

// serverGen is an implementation of the Go protocol buffer compiler's
// plugin architecture.  It generates bindings for gRPC support.
type serverGen struct {
	gen *generator.Generator
}

// Name returns the name of this plugin, "serverGen".
func (c *serverGen) Name() string {
	return "serverGen"
}

// map of import pkg name to unique name
type importServerPkg map[string]*pkgServerInfo

type pkgServerInfo struct {
	ImportPath string
	KnownType  string
	UniqueName string
}

var importPkgsByName = importServerPkg{
	...
	"grpc":        {ImportPath: "google.golang.org/grpc", KnownType: "serverGenConn"},
	"io":          {ImportPath: "io", KnownType: "Reader"},
	"iocodec":     {ImportPath: "github.com/gofunct/protoc-gen-cobra/iocodec", KnownType: "Encoder"},
...
var sortedServerImportPkgNames = make([]string, 0, len(importPkgsByName))

// Init initializes the plugin.
func (c *serverGen) Init(gen *generator.Generator) {
	c.gen = gen
	for k := range importPkgsByName {
		importPkgsByName[k].UniqueName = generator.RegisterUniquePackageName(k, nil)
		sortedServerImportPkgNames = append(sortedServerImportPkgNames, k)
	}
	sort.Strings(sortedServerImportPkgNames)
}

// P forwards to c.gen.P.
func (c *serverGen) P(args ...interface{}) { c.gen.P(args...) }

// Generate generates code for the services in the given file.
func (c *serverGen) Generate(file *generator.FileDescriptor) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}

	c.P("// Reference imports to suppress errors if they are not otherwise used.")
	for _, n := range sortedServerImportPkgNames {
	...
	c.P("const _ = ", importPkgsByName["grpc"].UniqueName, ".SupportPackageIsVersion", generatedServerCodeVersion)
	c.P()
...
func (c *serverGen) GenerateImports(file *generator.FileDescriptor, imports []*generator.FileDescriptor) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}
	c.P("import (")
...
// reservedserverGenName records whether a serverGen name is reserved on the serverGen side.
var reservedserverGenName = map[string]bool{
	// TODO: do we need any in gRPC?
}

// generateService generates all the code for the named service.
func (c *serverGen) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
...
var generateServerCommandTemplateCode = `
var _Default{{.Name}}serverGenCommandConfig = _New{{.Name}}serverGenCommandConfig()
type _{{.Name}}serverGenCommandConfig struct {
...
func _New{{.Name}}serverGenCommandConfig() *_{{.Name}}serverGenCommandConfig {
	c := &_{{.Name}}serverGenCommandConfig{}
	envconfig.Process("", c)
	return c
}
func (o *_{{.Name}}serverGenCommandConfig) AddFlags(fs *pflag.FlagSet) {
	fs.StringVarP(&o.ServerAddr, "server-addr", "s", o.ServerAddr, "server address in form of host:port")
	fs.StringVarP(&o.RequestFile, "request-file", "f", o.RequestFile, "serverGen request file (must be json, yaml, or xml); use \"-\" for stdin + json")
	...
	fs.DurationVar(&o.Timeout, "timeout", o.Timeout, "serverGen connection timeout")
	...
	fs.StringVar(&o.CertFile, "tls-cert-file", o.CertFile, "serverGen certificate file")
	fs.StringVar(&o.KeyFile, "tls-key-file", o.KeyFile, "serverGen key file")
...
var {{.Name}}serverGenCommand = &cobra.Command{
	Use: "{{.UseName}}",
}
func _Dial{{.Name}}() (*grpc.serverGenConn, {{.Name}}serverGen, error) {
	cfg := _Default{{.Name}}serverGenCommandConfig
	...
	return conn, New{{.Name}}serverGen(conn), nil
}
type _{{.Name}}RoundTripFunc func(cli {{.Name}}serverGen, in iocodec.Decoder, out iocodec.Encoder) error
func _{{.Name}}RoundTrip(sample interface{}, fn _{{.Name}}RoundTripFunc) error {
	...
	conn, serverGen, err := _Dial{{.Name}}()
	if err != nil {
		return err
	}
	defer conn.Close()
	return fn(serverGen, d, em.NewEncoder(os.Stdout))
}
`

var generateServerCommandTemplate = template.Must(template.New("cmd").Parse(generateServerCommandTemplateCode))

func (c *serverGen) generateCommand(servName string) {
	var b bytes.Buffer
	err := generateServerCommandTemplate.Execute(&b, struct {
		Name    string
		UseName string
	}{
		Name:    servName,
...
var generateServerSubcommandTemplateCode = `
var _{{.FullName}}serverGenCommand = &cobra.Command{
	Use: "{{.UseName}}",
...
Authenticate using the Authorization header (requires transport security):
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err := _{{.ServiceName}}RoundTrip(v, func(cli {{.ServiceName}}serverGen, in iocodec.Decoder, out iocodec.Encoder) error {
{{if .serverGenStream}}
...
func init() {
	{{.ServiceName}}serverGenCommand.AddCommand(_{{.FullName}}serverGenCommand)
	_Default{{.ServiceName}}serverGenCommandConfig.AddFlags(_{{.FullName}}serverGenCommand.Flags())
}
`

var generateServerSubcommandTemplate = template.Must(template.New("subcmd").Parse(generateServerSubcommandTemplateCode))

func (c *serverGen) generateSubcommand(servName string, file *generator.FileDescriptor, method *pb.MethodDescriptorProto) {
	/*
		if method.GetserverGenStreaming() || method.GetServerStreaming() {
			return // TODO: handle streams correctly
		}
	*/
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedserverGenName[methName] {
		methName += "_"
	}
	...
	var b bytes.Buffer
	err := generateServerSubcommandTemplate.Execute(&b, struct {
		Name         string
		UseName      string
		ServiceName  string
		FullName     string
		...
		InputType:    inputType,
		serverGenStream: method.GetServerStreaming(),
...
func inputServerNames(s string) (importName, inputPackage, inputType string) {
