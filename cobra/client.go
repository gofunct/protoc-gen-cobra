...
const generatedClientCodeVersion = 4

func init() {
	generator.RegisterPlugin(new(clientGen))
}

// clientGen is an implementation of the Go protocol buffer compiler's
// plugin architecture.  It generates bindings for gRPC support.
type clientGen struct {
	gen *generator.Generator
}

// Name returns the name of this plugin, "clientGen".
func (c *clientGen) Name() string {
	return "clientGen"
}

// map of import pkg name to unique name
type importClientPkg map[string]*pkgClientInfo

type pkgClientInfo struct {
	ImportPath string
	KnownType  string
	UniqueName string
}

var importClientPkgsByName = importClientPkg{
	...
	"grpc":        {ImportPath: "google.golang.org/grpc", KnownType: "clientGenConn"},
...
var sortedClientImportPkgNames = make([]string, 0, len(importPkgsByName))

// Init initializes the plugin.
func (c *clientGen) Init(gen *generator.Generator) {
	c.gen = gen
	for k := range importPkgsByName {
		importPkgsByName[k].UniqueName = generator.RegisterUniquePackageName(k, nil)
		sortedClientImportPkgNames = append(sortedClientImportPkgNames, k)
	}
	sort.Strings(sortedClientImportPkgNames)
}

// P forwards to c.gen.P.
func (c *clientGen) P(args ...interface{}) { c.gen.P(args...) }

// Generate generates code for the services in the given file.
func (c *clientGen) Generate(file *generator.FileDescriptor) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}

	c.P("// Reference imports to suppress errors if they are not otherwise used.")
	for _, n := range sortedClientImportPkgNames {
	...
	c.P("const _ = ", importPkgsByName["grpc"].UniqueName, ".SupportPackageIsVersion", generatedClientCodeVersion)
	c.P()
...
func (c *clientGen) GenerateImports(file *generator.FileDescriptor, imports []*generator.FileDescriptor) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}
	c.P("import (")
...
// reservedclientGenName records whether a clientGen name is reserved on the clientGen side.
var reservedclientGenName = map[string]bool{
	// TODO: do we need any in gRPC?
}

// generateService generates all the code for the named service.
func (c *clientGen) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
...
var generateClientCommandTemplateCode = `
var _Default{{.Name}}clientGenCommandConfig = _New{{.Name}}clientGenCommandConfig()
type _{{.Name}}clientGenCommandConfig struct {
...
func _New{{.Name}}clientGenCommandConfig() *_{{.Name}}clientGenCommandConfig {
	c := &_{{.Name}}clientGenCommandConfig{}
	envconfig.Process("", c)
	return c
}
func (o *_{{.Name}}clientGenCommandConfig) AddFlags(fs *pflag.FlagSet) {
	fs.StringVarP(&o.ServerAddr, "server-addr", "s", o.ServerAddr, "server address in form of host:port")
	fs.StringVarP(&o.RequestFile, "request-file", "f", o.RequestFile, "clientGen request file (must be json, yaml, or xml); use \"-\" for stdin + json")
	...
	fs.DurationVar(&o.Timeout, "timeout", o.Timeout, "clientGen connection timeout")
	...
	fs.StringVar(&o.CertFile, "tls-cert-file", o.CertFile, "clientGen certificate file")
	fs.StringVar(&o.KeyFile, "tls-key-file", o.KeyFile, "clientGen key file")
...
var {{.Name}}clientGenCommand = &cobra.Command{
	Use: "{{.UseName}}",
}
func _Dial{{.Name}}() (*grpc.clientGenConn, {{.Name}}clientGen, error) {
	cfg := _Default{{.Name}}clientGenCommandConfig
	...
	return conn, New{{.Name}}clientGen(conn), nil
}
type _{{.Name}}RoundTripFunc func(cli {{.Name}}clientGen, in iocodec.Decoder, out iocodec.Encoder) error
func _{{.Name}}RoundTrip(sample interface{}, fn _{{.Name}}RoundTripFunc) error {
	...
	conn, clientGen, err := _Dial{{.Name}}()
	if err != nil {
		return err
	}
	defer conn.Close()
	return fn(clientGen, d, em.NewEncoder(os.Stdout))
}
`

var generateClientCommandTemplate = template.Must(template.New("cmd").Parse(generateClientCommandTemplateCode))

func (c *clientGen) generateCommand(servName string) {
	var b bytes.Buffer
	err := generateClientCommandTemplate.Execute(&b, struct {
		Name    string
		UseName string
	}{
		Name:    servName,
...
var generateClientSubcommandTemplateCode = `
var _{{.FullName}}clientGenCommand = &cobra.Command{
	Use: "{{.UseName}}",
...
Authenticate using the Authorization header (requires transport security):
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		err := _{{.ServiceName}}RoundTrip(v, func(cli {{.ServiceName}}clientGen, in iocodec.Decoder, out iocodec.Encoder) error {
{{if .clientGenStream}}
...
func init() {
	{{.ServiceName}}clientGenCommand.AddCommand(_{{.FullName}}clientGenCommand)
	_Default{{.ServiceName}}clientGenCommandConfig.AddFlags(_{{.FullName}}clientGenCommand.Flags())
}
`

var generateClientSubcommandTemplate = template.Must(template.New("subcmd").Parse(generateClientSubcommandTemplateCode))

func (c *clientGen) generateSubcommand(servName string, file *generator.FileDescriptor, method *pb.MethodDescriptorProto) {
	/*
		if method.GetclientGenStreaming() || method.GetServerStreaming() {
			return // TODO: handle streams correctly
		}
	*/
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedclientGenName[methName] {
		methName += "_"
	}
	...
	var b bytes.Buffer
	err := generateClientSubcommandTemplate.Execute(&b, struct {
		Name         string
		UseName      string
		ServiceName  string
		FullName     string
		...
		InputType:    inputType,
		clientGenStream: method.GetClientStreaming(),
